...
bool Controller::reinitializeDevice() {
    ...
    // Temporarily stop controller to prevent callbacks and event processing
    isRunning_ = false;
    eventCv_.notify_all();
    {
        // Drop any pending events that may have been generated by old peripherals
        std::lock_guard<std::mutex> lock(eventQueueMutex_);
        std::queue<Event> emptyQueue;
        std::swap(eventQueue_, emptyQueue);
    }

    // Shutdown old peripherals; should not receive callbacks while not running
    shutdownPeripherals();
    ...
    // Resume normal operation only after peripherals are reinitialized
    if (ok) {
        isRunning_ = true;
