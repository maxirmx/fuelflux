...
void StateMachine::setupTransitions() {
    ...
    transitions_[{SystemState::RefuelDataTransmission, Event::DataTransmissionComplete}] = {SystemState::RefuelingComplete, [this]() { onEnableCardReading(); }};
...
void StateMachine::onErrorCancelPressed() {
    ...
    if (controller_) {
        ...
        if (ok) {
            // On successful reinit, post ErrorRecovery event to transition to Waiting
            controller_->postEvent(Event::ErrorRecovery);
        }
        // On failure, state remains Error (no event posted)
    }
}
